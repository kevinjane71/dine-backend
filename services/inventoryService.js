...
class InventoryService {
  ...
  async createDefaultRecipe(restaurantId, menuItemId, itemName, description, userId = 'system') {
    try {
      ...
      // Load existing inventory items FIRST — needed for the self-link check below AND for AI mapping.
      ...
      // ── SELF-LINK: direct-sale / packaged items are sold 1:1 from their OWN stock ──────────────
      // A Coke, a water bottle, packaged chips etc. are NOT "made from" ingredients — you sell one,
      // you deduct one. If the menu item's own name matches an existing inventory item, create a
      // DIRECT 1:1 recipe (deduct 1 unit per sale) and SKIP the AI entirely. This prevents the AI
      // from inventing a phantom ingredient (e.g. a "Coke@20" item wrongly getting a "Coca-Cola
      // 200ml" recipe that matches no real stock, so auto-consumption silently deducts nothing).
      const selfName = (itemName || '').toLowerCase().trim();
      const selfMatch = selfName ? inventoryItems.find(inv => (inv.name || '').toLowerCase().trim() === selfName) : null;
      if (selfMatch) {
        await db.collection('recipes').add({
          ...
          name: itemName,
          description: 'Direct 1:1 stock link',
          ingredients: [{ inventoryItemId: selfMatch.id, inventoryItemName: selfMatch.name, quantity: 1, unit: selfMatch.unit || 'pcs' }],
        ...
        console.log(`🔗 Direct 1:1 stock link for "${itemName}" → "${selfMatch.name}" (skipped AI recipe)`);
        return;
      }

      // Prepared item (no matching stock SKU by name) — generate the recipe via AI.
