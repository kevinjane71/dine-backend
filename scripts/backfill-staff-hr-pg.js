#!/usr/bin/env node
/**
 * backfill-staff-hr-pg.js — Migrate staffUsers, attendance, leaveRequests,
 * leaveConfig, leaveBalances, payrollConfig, payrollRuns, paySlips
 * from Firestore -> PG.
 *
 * Usage:
 *   node scripts/backfill-staff-hr-pg.js [--dry-run] [--upsert] [--collection <name>]
 */
...
const {
  ...
  attendanceToPgRow, ATTENDANCE_JSONB_COLUMNS,
  leaveRequestToPgRow, LEAVE_REQUEST_JSONB_COLUMNS,
  leaveConfigToPgRow, LEAVE_CONFIG_JSONB_COLUMNS,
  leaveBalanceToPgRow, LEAVE_BALANCE_JSONB_COLUMNS,
  payrollConfigToPgRow, PAYROLL_CONFIG_JSONB_COLUMNS,
  payrollRunToPgRow, PAYROLL_RUN_JSONB_COLUMNS,
  paySlipToPgRow, PAY_SLIP_JSONB_COLUMNS,
} = require('../repos/staffHrFieldMapper');
...
async function main() {
  console.log('=== Staff + HR Backfill: Firestore -> PG ===');
  ...
  const collections = [
    { name: 'staffUsers',             table: 'staff_users',      fn: staffToPgRow,         jsonb: STAFF_JSONB_COLUMNS },
    { name: 'attendance',             table: 'attendance',        fn: attendanceToPgRow,    jsonb: ATTENDANCE_JSONB_COLUMNS },
    { name: 'leaveRequests',          table: 'leave_requests',    fn: leaveRequestToPgRow,  jsonb: LEAVE_REQUEST_JSONB_COLUMNS },
    { name: 'leaveConfig',            table: 'leave_config',      fn: leaveConfigToPgRow,   jsonb: LEAVE_CONFIG_JSONB_COLUMNS },
    { name: 'leaveBalances',          table: 'leave_balances',    fn: leaveBalanceToPgRow,  jsonb: LEAVE_BALANCE_JSONB_COLUMNS },
    { name: 'payrollConfig',          table: 'payroll_config',    fn: payrollConfigToPgRow, jsonb: PAYROLL_CONFIG_JSONB_COLUMNS },
    { name: 'payrollRuns',            table: 'payroll_runs',      fn: payrollRunToPgRow,    jsonb: PAYROLL_RUN_JSONB_COLUMNS },
    { name: 'paySlips',               table: 'pay_slips',         fn: paySlipToPgRow,       jsonb: PAY_SLIP_JSONB_COLUMNS },
  ];
