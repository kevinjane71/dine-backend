#!/usr/bin/env node
/**
 * backfill-payments-pg.js — Migrate dine_payments, dine_subscriptions,
 * and dine_orders (→ dine_razorpay_orders) from Firestore → PG.
 *
 * Usage:
 *   node scripts/backfill-payments-pg.js [--dry-run] [--upsert] [--collection <name>]
 */
...
const {
  paymentsToPgRow, PAYMENTS_JSONB_COLUMNS,
  subscriptionsToPgRow, SUBSCRIPTIONS_JSONB_COLUMNS,
  ordersToPgRow, ORDERS_JSONB_COLUMNS,
} = require('../repos/paymentsFieldMapper');
...
async function main() {
  console.log('=== Payments Backfill: Firestore → PG ===');
  ...
  const collections = [
    { name: 'dine_payments',      table: 'dine_payments',         fn: paymentsToPgRow,      jsonb: PAYMENTS_JSONB_COLUMNS },
    { name: 'dine_subscriptions', table: 'dine_subscriptions',    fn: subscriptionsToPgRow,  jsonb: SUBSCRIPTIONS_JSONB_COLUMNS },
    { name: 'dine_orders',        table: 'dine_razorpay_orders',  fn: ordersToPgRow,         jsonb: ORDERS_JSONB_COLUMNS },
  ];
