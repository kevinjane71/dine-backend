...
-- ===== Offline sync foundation (M1-M4) appended for local PG =====
-- The pg_dump above runs `set search_path = ''` and qualifies its own tables as public.*,
-- but the CREATE TABLEs below are UNqualified. Without resetting the search_path they fail
-- with 3F000 "no schema has been selected", and because the whole file loads as one implicit
-- transaction that rolls back ALL 167 tables — leaving a fresh device with an empty schema.
